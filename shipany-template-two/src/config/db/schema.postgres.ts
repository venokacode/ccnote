...
export const stickyNote = table(
  'sticky_note',
  {
    ...
    type: text('type').notNull(), // missing_call | installation_record | general_inquiry | internal_task | other
    color: text('color').notNull(), // blue | red | yellow | green | purple
    status: text('status').notNull(), // new | in_progress | waiting | done | archived
  ...
  (table) => [
    // Query notes by org and status
    index('idx_sticky_note_org_status').on(table.orgId, table.status),
    // Query notes by org and type
    index('idx_sticky_note_org_type').on(table.orgId, table.type),
    // Order by zIndex for rendering
    index('idx_sticky_note_org_z_index').on(table.orgId, table.zIndex),
  ]
);

export const reminder = table(
  'reminder',
  {
    id: text('id').primaryKey(),
    orgId: text('org_id')
    ...
    noteId: text('note_id')
      .notNull()
      .references(() => stickyNote.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    // Query reminders by org and note
    index('idx_reminder_org_note').on(table.orgId, table.noteId),
    // Query upcoming reminders
    index('idx_reminder_org_triggered').on(
      table.orgId,
      table.isTriggered,
      table.remindAt
    ),
...
export const noteComment = table(
  'note_comment',
  {
    id: text('id').primaryKey(),
    orgId: text('org_id')
  ...
  (table) => [
    // Query comments by note (most common)
    index('idx_note_comment_org_note').on(table.orgId, table.noteId),
  ]
);

export const workspaceState = table('workspace_state', {
  orgId: text('org_id')
    .primaryKey()
    .references(() => organization.orgId, { onDelete: 'cascade' }),
  zoomLevel: integer('zoom_level').notNull().default(100), // 50-150
  viewportOffsetX: integer('viewport_offset_x').notNull().default(0),
  viewportOffsetY: integer('viewport_offset_y').notNull().default(0),
  updatedAt: timestamp('updated_at')
    .$onUpdate(() => new Date())
    .notNull(),
});
